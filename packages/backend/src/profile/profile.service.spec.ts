...
function makeTenantRecord(overrides: Partial<{
  id: string;
  name: string;
  slug: string;
  phone: string | null;
...
function makePrisma(overrides: {
  ...
  return {
    tenant: {
      findUniqueOrThrow: overrides.tenantFindUniqueOrThrow ?? jest.fn().mockResolvedValue(tenant),
      update: overrides.tenantUpdate ?? jest.fn().mockResolvedValue(tenant),
    },
    imageVersion: {
      findMany: overrides.imageVersionFindMany ?? jest.fn().mockResolvedValue([]),
    },
  } as any;
}

function makeGeocoder(geocodeImpl?: () => Promise<{ latitude: number; longitude: number } | null>) {
  return {
    geocode: jest.fn().mockImplementation(
      geocodeImpl ?? (() => Promise.resolve({ latitude: 19.4326, longitude: -99.1332 })),
    ),
  } as any;
}
...
describe('ProfileService — unit tests', () => {
  ...
  describe('updateProfile', () => {
    ...
    it('PATCH with invalid name length — service passes data through (validation is DTO layer)', async () => {
      ...
      const tenantUpdate = jest.fn().mockResolvedValue(makeTenantRecord({ name: 'X' }));
      const prisma = makePrisma({ tenantUpdate });
      const { service } = makeService(prisma);
  ...
  describe('updateLocation', () => {
    it('stores explicit valid coordinates directly without calling geocoder', async () => {
      const tenantUpdate = jest.fn().mockResolvedValue(
        makeTenantRecord({ address: 'Calle 1', latitude: 20.0, longitude: -100.0 }),
      );
      const prisma = makePrisma({ tenantUpdate });
    ...
    it('calls geocoder when no explicit coordinates are provided', async () => {
      ...
      expect(geocoder.geocode).toHaveBeenCalledWith('Paseo de la Reforma, CDMX');
    ...
    it('stores address with null coordinates and geocodingWarning when geocoder returns null', async () => {
      ...
      expect(tenantUpdate).toHaveBeenCalledWith({
        where: { id: 'tenant-123' },
        data: { address: 'Dirección desconocida 999', latitude: null, longitude: null },
      });
    ...
    it('throws UnprocessableEntityException for latitude out of range', async () => {
      ...
      await expect(service.updateLocation('tenant-123', dto)).rejects.toBeInstanceOf(
  ...
  describe('getImageHistory', () => {
    it('returns image versions ordered by createdAt desc', async () => {
      const versions = [
        makeImageVersionRecord({ id: 'v2', createdAt: new Date('2024-06-02T00:00:00Z') }),
        makeImageVersionRecord({ id: 'v1', createdAt: new Date('2024-06-01T00:00:00Z') }),
      ];
      const prisma = makePrisma({
        ...
        fc.record(
          {
            ...
            timezone: fc.option(
              fc.constantFrom('America/Mexico_City', 'America/Monterrey', 'America/Tijuana'),
              { nil: undefined },
            ),
          },
          { requiredKeys: [] },
...
describe('UpdateProfileDto — Property 11: Business name length validation', () => {
  it('rejects names shorter than 2 or longer than 100 characters', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.string({ minLength: 0, maxLength: 200 }).filter((s) => s.length < 2 || s.length > 100),
        async (invalidName) => {
          const dto = new UpdateProfileDto();
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (lat, lng) => {
          ...
          await expect(service.updateLocation('tenant-123', dto)).resolves.toBeDefined();
        ...
        async (address) => {
          ...
          expect(tenantUpdate).toHaveBeenCalledWith(
            expect.objectContaining({
              data: expect.objectContaining({ address, latitude: null, longitude: null }),
            }),
          );
        },
      ),
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (address, errorMessage) => {
          const geocoder = makeGeocoder(() => Promise.reject(new Error(errorMessage)));
          const tenantUpdate = jest.fn().mockResolvedValue(makeTenantRecord());
          const prisma = makePrisma({ tenantUpdate });
          const { service } = makeService(prisma, geocoder);
