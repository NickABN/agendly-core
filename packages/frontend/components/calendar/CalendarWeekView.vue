          ...
          <template v-if="(appointmentsByDate.get(wd.date)?.length ?? 0) > 0">
            ...
            >
              ...
              <div class="mt-2 flex items-center gap-1.5">
                <div class="w-2 h-2 rounded-full" :class="employeeColorByName.get(appt.employee.name) || 'bg-slate-400'" aria-hidden="true"></div>
                <span class="text-[10px] font-medium text-[var(--color-on-surface-variant)]">{{ appt.employee.name.split(' ')[0] }}</span>
              </div>
            </button>
          </template>
          <div v-else class="h-full flex items-center justify-center">
            <span v-if="wd.isSunday" class="material-symbols-outlined text-2xl text-[var(--color-outline-variant)]/30" aria-hidden="true">event_busy</span>
